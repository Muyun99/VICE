...
def do_python_eval(predict_folder, gt_folder, name_list, num_cls=21, input_type='png', threshold=1.0, printlog=False):
    ...
    for i in range(8):
        p = multiprocessing.Process(target=compare, args=(
            i, 8, TP, P, T, input_type, threshold))
        p.start()
        p_list.append(p)
    for p in p_list:
        p.join()
    ...
    for i in range(num_cls):
        ...
        FP_ALL.append((P[i].value-TP[i].value) /
                      (T[i].value + P[i].value - TP[i].value + 1e-10))
        FN_ALL.append((T[i].value-TP[i].value) /
                      (T[i].value + P[i].value - TP[i].value + 1e-10))
    loglist = {}
    for i in range(num_cls):
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--gt_dir", default='./VOC2012/SegmentationClass', type=str)
    ...
    if not args.curve:
        ...
        for i in range(60):
            t = i/100.0
            loglist = do_python_eval(
                args.predict_dir, args.gt_dir, name_list, 21, args.type, t)
        ...
        print(args.comment)
        print('Best background score: %.3f\tmIoU: %.3f%%' %
              (best_bg_score, best_mIoU))
        writelog(args.logfile, {
                 'mIoU': l, 'Best background score': best_bg_score, 'best_mIoU': best_mIoU}, args.comment)
