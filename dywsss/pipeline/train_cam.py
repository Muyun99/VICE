...
from dywsss.tool import pyutils, imutils, torchutils, visualization
import importlib
...
def train(args):
    ...
    if args.network == "ScaleNet50_SEAM":
        from dywsss.network.scalenet_SEAM import ScaleNet50_SEAM
        ...
        from dywsss.network.scalenet_SEAM import ScaleNet101_SEAM
        model = ScaleNet101_SEAM(args.structure, args.weights, [int(i) for i in args.dilations.split('_')], args.num_cls)
    else:
        model = getattr(importlib.import_module('dywsss.' + args.network), 'Net')(args.num_cls)
    ...
    if args.heatmap_root == '':
        train_dataset = dywsss.tool.data.VOC12ClsDataset(args.train_list, voc12_root=args.voc12_root, pseudo_gt=args.pseudo_list,
